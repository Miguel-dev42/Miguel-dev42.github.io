    ...
    return (
      <React.Fragment key={id}>
        <div className='flex w-full h-5'>
          <div className='flex'>
            <div className=' text-ubt-green'>miguel@Dell</div>
  ...
  handleCommands = (command, rowId) => {
    ...
    switch (main) {
      ...
      case 'pwd':
        ...
        result = str.replace('~', '/home/miguel');
        break;
      case 'code':
        if (words[0] === '.' || words.length === 0) {
          ...
          result =
            "Command '" +
            main +
            "' not found, or not yet implemented.<br>Available Commands:[ cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-miguel, todoist, trash, settings, sendmsg]";
        }
        break;
      case 'echo':
      ...
      case 'about-miguel':
        if (words[0] === '.' || words.length === 0) {
          this.props.openApp('about-miguel');
        } else {
          result =
            "Command '" +
            main +
